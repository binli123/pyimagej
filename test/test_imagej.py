...
def assert_labels_equality(exp: Dict[str, Any], act: Dict[str, Any], ignored_keys: List[str]):
    for key in exp.keys():
...
class TestImgLabelingConversions(object):
    ...
    def test_java_to_py(self, ij_fixture, py_labeling, java_labeling):
        ...
        assert_labels_equality(vars(exp_labels), vars(act_labels), ['numSources', 'indexImg'])
