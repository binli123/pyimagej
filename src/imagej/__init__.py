...
class ImageJPython:
    ...
    def __init__(self, ij):
        self._ij = ij
        sj.when_jvm_starts(_add_converters)
    ...
    def _can_convert_rai(self, obj) -> bool:
        ...
        try:
            return self._ij.convert().supports(obj, _RandomAccessibleInterval.fget())
        except Exception:
            return False
    ...
    def _can_convert_imgPlus(self, obj) -> bool:
        """Return false unless conversion to RAI is possible."""
        try:
            can_convert = self._ij.convert().supports(obj, _ImgPlus.fget())
    ...
    def _imagej_py_converters(self) -> List[sj.Converter]:
        ...
        return [
            sj.Converter(
                predicate=lambda obj: isinstance(obj, _ImgLabeling.fget()),
                converter=self._imglabeling_to_labeling,
                priority=sj.Priority.HIGH
            ),
            sj.Converter(
                predicate=lambda obj: _ImagePlus.fget() and isinstance(obj, _ImagePlus.fget()),
            ...
            sj.Converter(
                ...
                converter=lambda obj: self._permute_dataset_to_python(self._ij.convert().convert(obj, _ImgPlus.fget())),
                priority=sj.Priority.HIGH
            ),
            sj.Converter(
                predicate=self._can_convert_rai,
                converter=lambda obj: self._permute_rai_to_python(self._ij.convert().convert(obj, _RandomAccessibleInterval.fget())),
                priority=sj.Priority.HIGH - 2
            )
        ]
    ...
    def _dataset_to_xarray_permuted(self, rai):
